...
class Resolvers::SignInUserTest < ActiveSupport::TestCase
  ...
  setup do
    @user = User.create! name: 'test', email: 'test@email.com', password: 'testpwd123!'
  end
  ...
  test 'handling wrong email' do
    assert_nil perform(email: {email: 'wrong', password: @user.password})
  end

  test 'handle wrong password' do
